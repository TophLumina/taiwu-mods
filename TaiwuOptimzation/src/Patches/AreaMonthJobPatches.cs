...
[HarmonyPatch]
internal static class CharacterAreaActionDelayPatch
{
    ...
    private static bool Prefix(int areaId, ICharacterParallelAction action) =>
        !DelayMonthRuntime.TryDelayCharacterAreaAction(areaId, action);
}
...
internal static class MapBrokenBlockUpdateDelayPatch
{
    private static bool Prefix(int areaIdInt) =>
        !DelayMonthRuntime.TryDelayMapBrokenBlockUpdate(areaIdInt);
}
...
internal static class ExtraAnimalAreaDataDelayPatch
{
    private static bool Prefix(DataContext context) =>
        !DelayMonthRuntime.TryHandleAnimalAreaData(context);
}
...
{
    private static bool Prefix(DataContext context) =>
        !DelayMonthRuntime.TryHandleSkeletonGeneration(context);
}
...
{
    private static bool Prefix(DataContext context) =>
        !DelayMonthRuntime.TryDelayMapPickupCleanup(context);
}
