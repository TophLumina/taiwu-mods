return {
	BackendPlugins = {
		[1] = "TaiwuOptimization.dll",
	},
	FrontendPlugins = {
		[1] = "TaiwuOptimizationFront.dll",
	},
	Title = "[天幕心帷]过月性能优化",
	Version = "0.1.2.0",
	Author = "man!",
	Description = "过月性能优化模组\n通过将远离太吾的低风险过月计算任务延迟到过月后tick中分批执行，缩短过月伪死的时间。\n可中途添加，对原版存档无影响但是可能(大概率)与手动存档冲突",
	Source = 0,
	HasArchive = false,
	NeedRestartWhenSettingChanged = false,
	ChangeConfig = false,
	TagList = {
		[1] = "Compatible Mods",
		[2] = "Optimizations",
		[3] = "Configurations",
	},
	GameVersion = "1.0.20.0",
	DefaultSettings = {
		[1] = {
			SettingType = "Toggle",
			Key = "Enabled",
			DisplayName = "启用延迟过月",
			Description = "启用将远离太吾的低风险区域过月任务延迟到过月后帧中分批执行。",
			DefaultValue = true,
		},
		[2] = {
			SettingType = "Toggle",
			Key = "SyncNeighborStates",
			DisplayName = "相邻州域同步结算",
			Description = "相邻州域始终保持同步计算，不参与延迟。",
			DefaultValue = true,
		},
		[3] = {
			SettingType = "Slider",
			Key = "FrameBudgetMs",
			DisplayName = "每帧处理预算(毫秒)",
			GroupName = nil,
			MinValue = 1,
			MaxValue = 4,
			StepSize = 1,
			DefaultValue = 1,
		},
		[4] = {
			SettingType = "Toggle",
			Key = "DelayEquipment",
			DisplayName = "延迟NPC装备/修理",
			Description = "延迟非实时区域 NPC 的装备选择、修理和部分物品准备逻辑。",
			DefaultValue = true,
		},
		[5] = {
			SettingType = "Toggle",
			Key = "DelayMissionGoal",
			DisplayName = "延迟NPC行为规划",
			Description = "延迟非实时区域 NPC 的任务刷新和目标规划刷新。",
			DefaultValue = true,
		},
		[6] = {
			SettingType = "Toggle",
			Key = "DelayLoseOverloadItems",
			DisplayName = "延迟NPC超重丢弃",
			Description = "延迟非实时区域 NPC 的超重物品丢弃逻辑。",
			DefaultValue = true,
		},
		[7] = {
			SettingType = "Toggle",
			Key = "DelayBrokenBlocks",
			DisplayName = "延迟破损地块倒计时",
			Description = "延迟非实时区域破损地块的每月倒计时更新。",
			DefaultValue = true,
		},
		[8] = {
			SettingType = "Toggle",
			Key = "DelayAnimalAreaData",
			DisplayName = "延迟野生动物生态",
			Description = "延迟非实时区域的野生动物生成和自然死亡结算。",
			DefaultValue = true,
		},
		[9] = {
			SettingType = "Toggle",
			Key = "DelaySkeletonGeneration",
			DisplayName = "延迟墓地骷髅生成",
			Description = "延迟非实时区域的废墟墓地骷髅生成。",
			DefaultValue = true,
		},
		[10] = {
			SettingType = "Toggle",
			Key = "DelayMapPickups",
			DisplayName = "延迟拾取物状态刷新",
			Description = "延迟非实时区域的地图拾取可见性刷新和已忽略状态清理。",
			DefaultValue = true,
		},
	},
	FileId = 3750430637,
	Visibility = 0,
	SettingGroups = {
		[1] = "Default",
	},
	UpdateLogList = {
		[1] = {
			Timestamp = 1782234802,
		},
		[2] = {
			Timestamp = 1782235028,
		},
	},
	Cover = nil,
	WorkshopCover = nil,
}
