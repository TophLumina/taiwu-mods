return {
	...
	DefaultSettings = {
		...
		[15] = {
			...
			Description = "测试用选项，默认关闭。开启后按间隔向后端日志 Logs/GameData_*.log 写入多行聚合摘要，用于观察帧预算、pending 队列、超时和缓存构建情况；过月前/保存前强制清空 pending 时也会单独写入耗时和任务类型。如果出现频繁卡顿，可检查日志中是否有频繁的帧预算超时。正常游玩建议关闭。",
			GroupName = "运行时诊断日志",
			DefaultValue = false,
		},
		[16] = {
